...
:: External commands
if "%~1"=="status_zapret" (
    if exist "%ZROOT%utils\work_mode.enabled" (
        call :status_zapret_work
    ) else (
        call :test_service zapret soft
        call :test_service WinDivert soft
        call :tcp_enable
    )
    exit /b
)
...
if "%~1"=="check_updates" (
    ...
    if exist "%ZROOT%utils\check_updates.enabled" (
        if not "%~2"=="soft" (
...
if "%menu_choice%"=="12" goto setup_telegram
if "%menu_choice%"=="13" goto setup_telegram_vless
if "%menu_choice%"=="14" goto setup_autostart
if "%menu_choice%"=="15" goto setup_cursor_exclude
if "%menu_choice%"=="16" goto work_mode_switch
...
tasklist /FI "IMAGENAME eq winws.exe" | find /I "winws.exe" > nul
if !errorlevel!==0 (
    echo "winws.exe" is ALREADY RUNNING. Close it first if you want to restart.
    pause
    exit /b
)
exit /b

:work_mode_switch_status
if exist "%ZROOT%utils\work_mode.enabled" (
    set "WorkModeStatus=enabled"
) else (
    set "WorkModeStatus=disabled"
)
exit /b
...
chcp 437 > nul
cls
set "workFlag=%ZROOT%utils\work_mode.enabled"
if exist "%workFlag%" (
    echo Disabling work mode...
    del /f /q "%workFlag%"
    call :PrintYellow "Windows service can be used again (if not blocked by Secret Net Studio)."
) else (
    echo Enabling work mode...
    echo enabled>"%workFlag%"
    call :PrintGreen "Work mode ON: no Windows service, use general (ALT11).bat or autostart."
    call :PrintYellow "Recommended on corporate PCs with Secret Net Studio."
)
pause
goto menu
...
if exist "%ZROOT%utils\work_mode.enabled" (
    echo.
    call :PrintYellow "Work mode is enabled - Windows service is disabled on this PC."
    call :PrintYellow "Use option 14 (autostart) or run general (ALT11).bat instead."
    echo.
    pause
    goto menu
)
            ...
            if not "!arg!"=="^" (
                ...
                if "!arg:~0,1!" EQU "!QUOTE!" (
                    ...
                    if !errorlevel!==0 (
                        ...
                        set "arg=\!QUOTE!@!ZSCR!!arg:~1!\!QUOTE!"
                    ) else if "!arg:~0,5!"=="%%BIN%%" (
                        set "arg=\!QUOTE!!BIN_PATH!!arg:~5!\!QUOTE!"
                    ) else if "!arg:~0,7!"=="%%LISTS%%" (
                        set "arg=\!QUOTE!!LISTS_PATH!!arg:~7!\!QUOTE!"
                    ) else (
                        set "arg=\!QUOTE!!ZSCR!!arg!\!QUOTE!"
                    )
...
:: Secret Net Studio / corporate policy
set "secretNetFound=0"
sc query | findstr /I "Secret" > nul
if !errorlevel!==0 set "secretNetFound=1"
sc query | findstr /I "SnPolicy" > nul
if !errorlevel!==0 set "secretNetFound=1"
if !secretNetFound!==1 (
    call :PrintYellow "[?] Secret Net Studio (or similar) detected."
    call :PrintYellow "Windows service for zapret may be blocked. Use work mode (menu 16) and autostart (menu 14)."
) else (
    call :PrintGreen "Corporate security check passed"
)
echo:
...
set "gameFlagFile=%ZROOT%utils\game_filter.enabled"
...
set "checkUpdatesFlag=%ZROOT%utils\check_updates.enabled"
...
set "listFile=%ZROOT%lists\ipset-all.txt"
...
chcp 65001 > nul
cls
powershell -NoProfile -ExecutionPolicy Bypass -File "%ZROOT%utils\launcher.ps1" -Action start
goto menu

:setup_telegram_vless
chcp 65001 > nul
cls
powershell -NoProfile -ExecutionPolicy Bypass -File "%ZROOT%utils\launcher.ps1" -Action start
goto menu
...
echo.
start "" powershell -NoProfile -ExecutionPolicy Bypass -File "%ZROOT%utils\test-zapret.ps1"
pause
goto menu
...
if not exist "%ZROOT%bin\" set "extracted=0"
